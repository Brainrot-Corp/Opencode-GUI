...
- 2026-08-26: **Integrated terminal** — bottom dock inside `.main` (VS Code-style), toggle via composer button next to diff + `Ctrl+\`` (physical Backquote, layout-independent). Rust: new `pty.rs` module on `portable-pty` 0.9 (ConPTY) — `pty_spawn/write/resize/kill`, single session, output coalesced into >=8ms/64KB frames and streamed base64 as generation-tagged `pty://frame` events (`{g, d}`) — the frontend drops frames from superseded generations so dead shells can't bleed into the live view; exit on `pty://exit`; killed on app exit alongside the server child. Frontend: `Terminal.tsx` keeps xterm mounted for app lifetime (close = height collapse + **shell killed — no background terminals**; reopen spawns fresh), first open lazily boots + spawns shell (`$SHELL` → powershell.exe) at workspace cwd; workspace switch respawns (cwd baked at spawn). Theme: ANSI palette derived from syntax tokens, re-derived on `themes://changed`, transparent background over dock glass. Ctrl+Shift+C/V clipboard; Escape inside terminal goes to the shell, not the stop gesture. Persisted: `oc.term.open`, `oc.term.h`. Skipped until needed: tabs/splits, shell picker setting, search addon.
- 2026-08-27: **Per-session model memory** — picker stays global (`oc.lastModel`), but each session remembers the model it last used (`oc.sessionModels` map) and re-applies it on switch. Memory is written on explicit picks (including "" = server default clears it) and on steered replies (ground-truth `providerID/modelID` from `message.updated`, keyed by an equality guard so background sessions can't misattribute). Applying is reachability-gated against the live provider list: vanished models are skipped and pruned from the map, falling back to the global. `useProviders` gains the map + an apply/prune effect on `[activeId, providers, sessionModels]`; `useOpencode` exposes the combined pick entry (`setModelSel` → remember + apply) and mirrors `modelSel` through a ref for the boot-time SSE closure.
