...
export default function FileEditorHost({ hotkeys }: { hotkeys?: Record<string, string | null> }) {
  const [openPath, setOpenPath] = useState<{ path: string; absolute: string; dir: string } | null>(null);
    ...
    const onOpen = async (ev: Event) => {
      ...
      if (!abs) {
        try {
          ...
          if (wt) abs = toOpPath(`${wt}/${d.path}`, dir);
        } catch {}
      }
      dirtyRef.current = false;
      setOpenPath({ path: d.path, absolute: abs, dir });
    };
  ...
  return (
    <FileEditor
      key={`${openPath.dir}\0${openPath.path}`}
      path={openPath.path}
