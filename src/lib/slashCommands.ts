  ...
  if (id) {
    switch (name) {
      ...
      case "compact": {
        ...
        if (!sel) return true;
        const dir = ctx.getDirForSession?.(id) ?? "";
        if (!isModelOnServer(sel, dir)) {
          ctx.setError(`Model ${sel} isn't on this server — pick one it has to compact.`);
          return true;
        }
